...
one). Builds on the charter edition, so read that first.
...
Run this periodically (for example every 60 to 90 minutes, or every N experiments per agent):
...
2. **Crown the champion.** The global champion is simply the branch holding the
   lowest `val_bpb`, since one comparable number makes this trivial.
